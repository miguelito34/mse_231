...
Here we do simple string manipulation to try and clear the names of any numbers and emojis under the assumption that first names only include alpha characters. Once we have isolated estimated first names we join our tweet data with our
gender data so that we can attempt to guess the gender of users.

We make an educated guess by determining which gender a name occurs with more. The formula used is (M(x) - F(x))/(M(x) + F(x)). For names present in the data, this gives us a number from -1 to 1, with -1 indicating female, 0 indicating unkown, and 1 indicating male.
```{r}
find_gender <- function(data) {
  ...
  data %>% 
    ...
    # inferring gender, connects each name to counts, and infers gender
    ...
    # Collects new data and then spreads it out so that further analysis can be
    # done, keeping interacting pairs together
