...
###  Parameters
```{r}
all_tweets_pathname <- "all_tweets_clean_24hrs.tsv"
...
all_tweets_gen %>% 
  ...
  group_by(user_est_gender, date, time) %>% 
  count() %>% 
  arrange(date, time, user_est_gender)
```

Tweet Volume by Gender for Filtered Tweets

The number of users who's genders were arbitrarily guessed is 47, thus insignificant.
```{r fig.height=2, fig.width=3}
filtered_tweets_gen %>% 
  filter(user_est_gender != "unknown") %>% 
  group_by(date, time, user_est_gender) %>% 
  count(name = "tweets") %>% 
  arrange(date, time, user_est_gender) %>% 
  ungroup() %>% 
  transmute(
  ...
  labs(
    title = "Tweet Volume by Gender for Filtered Tweets",
    x = NULL,
    y = "Tweet Volume",
