...
# Retireves the date, hour, and hack from the group key
...
def break_in_same_hour(current_pickup_time, prev_dropoff_time):
    current_pickup_time = pd.to_datetime(current_pickup_time, format = '%Y-%m-%d %H:%M:%S')
    prev_dropoff_time = pd.to_datetime(prev_dropoff_time, format = '%Y-%m-%d %H:%M:%S')
    return (current_pickup_time.hour == prev_dropoff_time.hour)


# Calculates and partitions the time on-duty (t_onduty) dependent on the span of the break as found in "break_in_same_hour"
def non_occupied_t_onduty(current_pickup_time, prev_dropoff_time, calc_for_hour):
    ...
    if (break_time < .5):
        if (calc_for_hour == "same"):
            ...
            return float((prev_dropoff_time.ceil(freq = "H") - prev_dropoff_time).total_seconds()/3600)
        else: 
            # Partitions time and adds to t_onduty sum for current hour
            return float((current_pickup_time - current_pickup_time.floor(freq = "H")).total_seconds()/3600)
    else:
        return 0.0
...
# Formats and prints the output line       
